...
Critical paths are protected by deterministic Node tests for one-shot OAuth state/tickets, raw-profile minimization, transactional SID creation, independent preference/affect scoring, pseudonymous utterance/persona export, viewer-reaction aggregation, beat validation, and time-series DesignGap. The React login completion route must also pass a production Vite build.
